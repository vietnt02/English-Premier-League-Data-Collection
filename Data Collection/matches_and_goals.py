...
# Create a list to store all existing match_id values
cursor.execute('SELECT match_id FROM dbo.matches')
...
for match_ids in match_id_list():
    ...
    for match_id in match_ids:
        # Skip the loop if match_id already exists
        if match_id in existing_match_ids:
            print("Match ID {match_id} already exists. Skipping the loop.")
            continue
